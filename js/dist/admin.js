...
var SettingsModal = _fof_components__WEBPACK_IMPORTED_MODULE_0__["settings"].SettingsModal,
    BooleanItem = _fof_components__WEBPACK_IMPORTED_MODULE_0__["settings"].items.BooleanItem;
var networks = [['facebook', 'twitter', 'linkedin', 'reddit', 'whatsapp'], ['vkontakte', 'odnoklassniki', 'my_mail'], ['qq', 'qzone']];
app.initializers.add('fof/share-social', function () {
  app.extensionSettings['fof-share-social'] = function () {
    return app.modal.show(new SettingsModal({
      ...
      size: 'small',
      items: [].concat(networks.map(function (networks) {
        return networks.map(function (network) {
          return m(BooleanItem, {
            key: "fof-share-social.networks." + network
